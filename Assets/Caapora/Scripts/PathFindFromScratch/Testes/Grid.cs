...
namespace PathFindingTeste {
     ...
     //   worldBottomLeft = Quaternion.Euler(-90, 0, 0) * worldBottomLeft;
		...
		for (int x = 0; x < gridSizeX; x ++) {
			for (int y = 0; y < gridSizeY; y ++) {
                    ...
                    Debug.Log("grid[x]: " + worldPoint.x);
                    Debug.Log("grid[y]: " + worldPoint.y);
	...
	public Node NodeFromWorldPoint(Vector3 worldPosition) {
         ...
         //   gridWorldSize = new Vector3(270, 270, 1);
		...
		percentY = Mathf.Clamp01(percentY);
            ...
            Debug.Log("gridPathX : " + x);
            Debug.Log("gridPathY : " + y);
